...
export const OutputPromptsPanel: React.FC<OutputPromptsPanelProps> = ({
    ...
    const toggleContext = (sceneNum: string) => {
    ...
    return (
        <div className="overflow-y-scroll custom-scrollbar pl-1 space-y-2" style={{ width: `calc(${100 - verticalDividerPos}% - 0.5rem)`, contentVisibility: 'auto' }} onWheel={e => e.stopPropagation()}>
            ...
            {groupedPrompts.length > 0 ? groupedPrompts.map((group) => {
                ...
                const cleanTitle = (group.title || '').replace(/^(Scene|Сцена|Scène|Escena|Szene)\s*\d+[:\s-]*/i, '').trim();
                const isContextExpanded = expandedContexts.has(group.sceneNum);
                    ...
                    <div key={group.sceneNum} className="mb-4">
                        <div className="px-2 py-1 mb-1 text-xs font-bold text-gray-400 uppercase border-b border-gray-700 flex justify-between items-center group cursor-pointer hover:bg-gray-800/50 transition-colors rounded" onClick={() => onToggleOutputSceneCollapse(group.sceneNum)}>
                            <div className="flex items-center space-x-2 overflow-hidden flex-1 min-w-0">
                                ...
                                <span className="truncate">{`${t('node.content.scene')} ${group.sceneNum}${cleanTitle ? `: ${cleanTitle}` : ''}`}</span>
                            </div>
                        ...
                        {!isSceneCollapsed && (
                            <div className="pl-2 space-y-2">
                                ...
                                <div className="bg-gray-800/80 border border-emerald-500/30 rounded-md mb-2 shadow-inner overflow-hidden transition-all">
                                    <div 
                                        className="flex justify-between items-center p-2 cursor-pointer hover:bg-gray-700/30 transition-colors"
                                        onClick={(e) => { e.stopPropagation(); toggleContext(group.sceneNum); }}
                                    >
                                        <div className="flex items-center gap-1.5">
                                            <div className={`text-emerald-500 transition-transform duration-200 ${isContextExpanded ? 'rotate-90' : ''}`}>
