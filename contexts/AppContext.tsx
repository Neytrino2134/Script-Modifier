...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const { currentCatalogItems, catalogPath, navigateCatalogBack, navigateCatalogToFolder, createCatalogItem, saveGroupToCatalog, saveGenericItemToCatalog, renameCatalogItem, deleteCatalogItem, saveCatalogItemToDisk, catalogFileInputRef, handleCatalogFileChange, triggerLoadFromFile, moveCatalogItem, catalogItems, activeCategory, setActiveCategory, importCatalog, mergeCatalogItems } = useCatalog(t);
    const { libraryItems, currentLibraryItems, currentPath: libraryPath, navigateBack, navigateToFolder, createLibraryItem, updateLibraryItem, deleteLibraryItem, saveLibraryItemToDisk, libraryFileInputRef, handleLibraryFileChange, triggerLoadLibraryFromFile, moveLibraryItem, importLibrary, mergeLibraryItems } = usePromptLibrary(t);
        ...
        setTabs(currentTabs => {
            ...
            const performClose = () => {
                 ...
                 setActiveTabIndex(prevIndex => {
                     ...
                     if (index === prevIndex) return index > 0 ? index - 1 : 0;
                     return prevIndex;
                 });
            };

            if (tabToClose.nodes.length > 0) {
                setConfirmInfo({
                    title: t('dialog.closeTab.title'),
                    message: t('dialog.closeTab.message', { tabName: tabToClose.name }),
            ...
            return currentTabs.filter((_, i) => i !== index);
        });
    }, [setConfirmInfo, t]);
    ...
    const contextValue: AppContextType = {
        ...
        fileInputRef, catalogFileInputRef, libraryFileInputRef, nodeIdCounter, // ADDED nodeIdCounter to export
